...
return new class extends Migration
{
    ...
    public function up(): void
    {
		Schema::create('customers', function (Blueprint $table) {
             ...
             *
             * Я даю пример чтобы было немного понятнее:
             *
             * Пример реализации:
             * $identifier = $phone . '|' . $email;  - объединяем в один ключ
             *
             * !Если вдруг у вас будут вопросы а почему бы просто взять за рснову users.id
             * я отвечаю что это не очень хорощая идея и вот почему:
             * Моделируем ситуацию:
             * Создаеться запись клиента -> отправляет заявку -> удаляет запись - И всеэто в течении суток
             * далее удаляем клиента заново -> cоздаеться запись клиента -> отправляет заявку -> удаляет запись - И всеэто в течении суток
             * тоесть за сутки можно таким образом отправить много заявок - Моя цель безопастность!
             *
             * Ну и логика проверок я ее вижу так:
             * При проверке новой заявки смотрим: есть ли уже запись с этим идентификатором за сегодня.
             * - Если есть - отклоняем заявку.
             * - Если нет - сохраняем и разрешаешь.
             */
